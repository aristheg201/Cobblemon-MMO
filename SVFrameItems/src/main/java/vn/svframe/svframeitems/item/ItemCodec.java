...
public final class ItemCodec {
    ...
    public static Optional<ItemInstance> read(ItemStack stack) {
        ...
        try {
            ...
            return Optional.of(new ItemInstance(instance, data.getString("definition-id"), data.getString("type-id"), data.getString("rarity-id"),
                    data.getInt("item-level"), data.getInt("upgrade-level"), data.getInt("definition-revision"), data.getLong("seed"), data.getLong("state-revision"), stats, sockets));
    ...
    private static NbtCompound socket(SocketState socket) {
        ...
        if (socket.gem() != null) {
            ...
            NbtList stats = new NbtList(); for (ItemStat stat : gem.stats()) stats.add(stat(stat)); gemNbt.put("stats", stats); nbt.put("gem", gemNbt);
        }
        return nbt;
    }
    private static SocketState readSocket(NbtCompound nbt) {
        ...
        EmbeddedGem gem = new EmbeddedGem(UUID.fromString(gemNbt.getString("instance-id")), gemNbt.getString("definition-id"), gemNbt.getString("type-id"), gemNbt.getString("rarity-id"), gemNbt.getInt("item-level"), gemNbt.getInt("upgrade-level"), gemNbt.getInt("definition-revision"), gemNbt.getLong("seed"), gemNbt.getLong("state-revision"), gemNbt.getString("color"), stats);
