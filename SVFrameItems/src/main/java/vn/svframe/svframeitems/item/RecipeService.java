...
public final class RecipeService {
    ...
    public boolean canCraft(Inventory inventory,String recipeId){RecipeDefinition recipe=registry.recipe(recipeId);return recipe!=null&&plan(inventory,recipe)!=null;}
    public Result craft(ServerPlayerEntity player,String recipeId){return craft(player.getInventory(),recipeId);}
    public Result craft(Inventory inventory,String recipeId){RecipeDefinition recipe=registry.recipe(recipeId);if(recipe==null)return new Result(Status.UNKNOWN_RECIPE,ItemStack.EMPTY);List<Consumption> plan=plan(inventory,recipe);if(plan==null)return new Result(Status.MISSING_INGREDIENTS,ItemStack.EMPTY);for(Consumption c:plan)inventory.getStack(c.slot()).decrement(c.count());ItemStack output=generator.generate(recipe.outputItemId(),recipe.outputLevel());output.setCount(recipe.outputAmount());inventory.markDirty();return new Result(Status.SUCCESS,output);}
    private static List<Consumption> plan(Inventory inventory,RecipeDefinition recipe){List<Consumption> all=new ArrayList<>();Map<Integer,Integer> reserved=new HashMap<>();for(RecipeDefinition.Ingredient ingredient:recipe.ingredients()){int remaining=ingredient.count();for(int slot=0;slot<inventory.size()&&remaining>0;slot++){ItemStack stack=inventory.getStack(slot);if(stack.isEmpty()||!matches(stack,ingredient))continue;int available=stack.getCount()-reserved.getOrDefault(slot,0);if(available<=0)continue;int take=Math.min(available,remaining);reserved.merge(slot,take,Integer::sum);remaining-=take;}if(remaining>0)return null;}reserved.forEach((slot,count)->all.add(new Consumption(slot,count)));return all;}
    private static boolean matches(ItemStack stack,RecipeDefinition.Ingredient ingredient){if(ingredient.kind()==RecipeDefinition.IngredientKind.SVFRAME_ITEM)return ItemCodec.read(stack).map(item->item.definitionId().equals(ingredient.id())).orElse(false);if(ItemCodec.isSVFrameItem(stack))return false;return Registries.ITEM.getId(stack.getItem()).toString().equals(ingredient.id());}
