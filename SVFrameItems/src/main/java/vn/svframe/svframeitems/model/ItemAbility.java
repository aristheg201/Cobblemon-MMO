...
public record ItemAbility(Trigger trigger, String skill, double chance, int cooldownTicks, Map<String,Double> parameters) {
    public enum Trigger { ATTACK, USE, EQUIP, UNEQUIP }
    public ItemAbility {
        ...
        if (cooldownTicks != 0) throw new IllegalArgumentException("Item ability cooldowns belong to SVFrameLib skill runtime; cooldown-ticks must be 0");
