package io.lumine.mythic.lib.metrics.bukkit;
import io.lumine.mythic.lib.module.MMOPlugin; import java.util.*; import java.util.concurrent.Callable;
public class Metrics {
    ...
    public io.lumine.mythic.lib.gson.JsonObject getPluginData(){io.lumine.mythic.lib.gson.JsonObject out=new io.lumine.mythic.lib.gson.JsonObject();out.addProperty("pluginName",plugin.getNamespacedKey());out.addProperty("enabled",enabled);io.lumine.mythic.lib.gson.JsonArray data=new io.lumine.mythic.lib.gson.JsonArray();for(CustomChart chart:charts){try{Object value=chart.getChartData();if(value==null)continue;io.lumine.mythic.lib.gson.JsonObject c=new io.lumine.mythic.lib.gson.JsonObject();c.addProperty("chartId",chart.chartId);c.addProperty("value",String.valueOf(value));data.add(c);}catch(Exception ignored){}}out.add("customCharts",data);return out;}
