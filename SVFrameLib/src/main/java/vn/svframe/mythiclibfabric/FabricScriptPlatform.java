...
final class FabricScriptPlatform implements ScriptPlatform {
    ...
    public boolean hasPermission(UUID target, String permission) {
        ...
        return player != null && MythicLibPermissionBridge.has(player, permission);
    }
    ...
    public void addStat(UUID target, String stat, String key, double value, boolean relative, boolean unique, long lifetimeTicks) {
        ...
        long expires = lifetimeTicks <= 0 ? Long.MAX_VALUE :
                (lifetimeTicks > Long.MAX_VALUE - MythicLibFabricMod.currentTick() ? Long.MAX_VALUE : MythicLibFabricMod.currentTick() + lifetimeTicks);
        MythicLibStatMod.engine().register(target, stat, new NativeStatEngine.Modifier(id, key, value, type,
    ...
    public void removeStat(UUID target, String stat, String key) {
        ...
        MythicLibStatMod.engine().removeByKey(target, stat, key);
    }
    ...
    @Override public void delay(int ticks, Runnable runnable) { MythicLibFabricMod.schedule(Math.max(0, ticks), runnable); }
