package vn.svframe.svframelib.message;
import vn.svframe.svframelib.api.player.MMOPlayerData; import net.minecraft.server.network.ServerPlayerEntity; import net.minecraft.text.Text; import net.minecraft.util.Formatting;
import java.util.*;
public abstract class PlayerMessage {
    ...
    public static PlayerMessage fromConfig(Object value){if(value==null)return new vn.svframe.svframelib.message.type.EmptyMessage();if(value instanceof PlayerMessage p)return p;if(value instanceof Map<?,?> m){Map<String,Object>x=new LinkedHashMap<>();m.forEach((k,v)->x.put(String.valueOf(k),v));String msg=String.valueOf(x.getOrDefault("message",x.getOrDefault("text","")));return new TextMessage(x,msg);}return new TextMessage(Map.of(),String.valueOf(value));}
