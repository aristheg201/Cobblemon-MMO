...
import vn.svframe.svframelib.version.Attributes;
...
public class Argument<T> {
    ...
    public static final Argument<String> COOLDOWN_CURRENT = new Argument<>("cooldown_key", (explorer, out) -> {
        Entity entity = explorer.getSender().getEntity();
        if (entity instanceof ServerPlayerEntity player && vn.svframe.svframelib.api.player.MMOPlayerData.has(player)) {
            var keys = vn.svframe.svframelib.api.player.MMOPlayerData.get(player).getCooldownMap().getCooldownKeys();
