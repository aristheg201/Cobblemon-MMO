...
/** CI smoke for bundled skill definition count and real SVFrameLib/external handler ownership. */
public final class YamlSkillCountSmoke {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("DEFAULT_SKILL_SOURCES=" + defaultSources + ",SVFRAMELIB_NATIVE=" + nativeSources + ",EXTERNAL_PROVIDER=" + externalSources);
        if (!missing.isEmpty()) throw new IllegalStateException("Unowned default skill sources: " + missing);

        // Bundled 1.7.1 default_skills.yml has 93 definitions, but only 90 are SVFrameLib classes.
        // AMBERS, NEPTUNE_GIFT and SNEAKY_PICKY are SVFrameMMO-owned registrations and must never be faked by SVFrameLib.
        if (actual == 93 && defaultSources == 93) {
            if (nativeSources != 90 || externalSources != 3)
                throw new IllegalStateException("Expected ownership 90 SVFrameLib + 3 external, got " + nativeSources + " + " + externalSources);
