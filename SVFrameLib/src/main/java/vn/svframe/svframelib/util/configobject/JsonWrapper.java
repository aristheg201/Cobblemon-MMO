...
public class JsonWrapper extends MapConfigObject {
    ...
    private static java.util.List<String> splitTopLevel(String s,char delimiter){java.util.List<String> out=new java.util.ArrayList<>();int start=0,depth=0;char quote=0;for(int i=0;i<s.length();i++){char c=s.charAt(i);if(quote!=0){if(c==quote&&(i==0||s.charAt(i-1)!='\\'))quote=0;continue;}if(c=='\''||c=='\"'){quote=c;continue;}if(c=='{'||c=='['||c=='(')depth++;else if(c=='}'||c==']'||c==')')depth--;else if(c==delimiter&&depth==0){out.add(s.substring(start,i).trim());start=i+1;}}out.add(s.substring(start).trim());return out;}
