...
import vn.svframe.svframelib.fabric.mixin.TextDisplayEntityAccessor;
...
public final class SVFrameLibIndicatorManager {
    ...
    private SVFrameLibIndicatorManager() { }
    ...
    private static String format(NativeDamageMetadata metadata, Indicator indicator) {
        ...
        String value;
        synchronized (SVFrameLibIndicatorManager.class) { value = decimals.format(indicator.value()); }
    ...
    private static String icon(Indicator indicator, boolean critical) {
        ...
        if (indicator.element() != null) {
            NativeElementRegistry.Element element = SVFrameLibCombatRuntime.elements().get(indicator.element());
            if (element != null) {
                if (!first) builder.append(iconJoin);
    ...
    private static void spawn(ServerWorld world, LivingEntity target, String formatted) {
        ...
        ((TextDisplayEntityAccessor) (Object) display).svframelib$setText(legacyText(formatted));
        ((DisplayEntityAccessor) (Object) display).svframelib$setBillboardMode(DisplayEntity.BillboardMode.CENTER);
        ...
        Vec3d velocity = move
                ...
                : Vec3d.ZERO;
        ACTIVE.add(new Active(display, velocity, gravity, move, tickPeriod, SVFrameLibFabricMod.currentTick() + lifespan));
    }
