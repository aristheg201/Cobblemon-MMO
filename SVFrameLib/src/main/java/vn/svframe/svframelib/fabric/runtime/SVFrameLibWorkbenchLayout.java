...
/** Exact slot topology used by SVFrameLib 1.7.1 Super/Mega Workbench mappings. */
public final class SVFrameLibWorkbenchLayout {
    ...
    private SVFrameLibWorkbenchLayout(Kind kind, int size, int width, int height, int[] inputSlots, int[] edgeSlots) {
        this.kind = kind;
        this.size = size;
        this.width = width;
        this.height = height;
    ...
    public static SVFrameLibWorkbenchLayout superWorkbench() {
        return new SVFrameLibWorkbenchLayout(Kind.SUPER, 45, 5, 5, SUPER_INPUT, SUPER_EDGE);
    }

    public static SVFrameLibWorkbenchLayout megaWorkbench() {
        return new SVFrameLibWorkbenchLayout(Kind.MEGA, 54, 6, 6, MEGA_INPUT, MEGA_EDGE);
    }
