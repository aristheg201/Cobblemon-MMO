...
import vn.svframe.svframelib.fabric.runtime.session.CooldownInfoRuntime;
...
/** Native Brigadier implementation of SVFrameLib 1.7.1 command surface. */
final class SVFrameLibCommands {
    private static final String ADMIN = "svframelib.admin";
    private static final String HEALTH_SCALE = "svframelib.svframelib.command.healthscale";
    private static final String TEMP_STAT = "svframelib.tempstat";

    private SVFrameLibCommands() {}

    static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(main("svframelib"));
    ...
    private static LiteralArgumentBuilder<ServerCommandSource> debugTree() {
        ...
        debug.then(literal("info").executes(ctx -> { success(ctx.getSource(), "SVFrameLib Fabric | " + SVFrameLibFabricMod.definitionSummary() + " | " + FabricDamageBridge.summary()); return 1; }));
        debug.then(literal("versions").executes(ctx -> { success(ctx.getSource(), "SVFrameLib 1.7.1 behavior port | Minecraft 1.21.1 | Fabric"); return 1; }));
        debug.then(literal("parse").then(argument("text", StringArgumentType.greedyString()).executes(ctx -> {
            ServerPlayerEntity player = ctx.getSource().getPlayerOrThrow();
            String parsed = vn.svframe.svframelib.fabric.runtime.NativePlaceholderRegistry.parse(player.getUuid(), StringArgumentType.getString(ctx, "text"));
    ...
    private static int reload(ServerCommandSource source) {
        boolean ok = SVFrameLibFabricMod.reloadAll();
        if (!ok) { source.sendError(Text.literal("SVFrameLib reload failed. Check server log.")); return 0; }
        success(source, "SVFrameLib reloaded | " + SVFrameLibFabricMod.definitionSummary() + " | " + FabricDamageBridge.summary());
        return 1;
    }
    ...
    private static int cast(ServerCommandSource source, String skill, ServerPlayerEntity target)
        ...
        boolean ok = SVFrameLibFabricMod.castSkill(skill, caster.getUuid(), actualTarget.getUuid(), Map.of());
    ...
    private static int statCheck(ServerCommandSource source, ServerPlayerEntity player, String stat) {
        NativeStatEngine.StatInstance instance = SVFrameLibStatMod.engine().instance(player.getUuid(), stat);
    ...
    private static int statAdd(ServerCommandSource source, ServerPlayerEntity player, String stat, String encoded, long duration, String key, boolean unique) {
        ...
        long expires = duration > 0L ? saturatingAdd(SVFrameLibFabricMod.currentTick(), duration) : Long.MAX_VALUE;
        SVFrameLibStatMod.engine().register(player.getUuid(), stat, new NativeStatEngine.Modifier(id, actualKey, parsed.value(), parsed.type(), NativeStatEngine.EquipmentSlot.OTHER, NativeStatEngine.ModifierSource.OTHER, expires));
    ...
    private static int statRemove(ServerCommandSource source, ServerPlayerEntity player, String stat, String key) {
        int removed = SVFrameLibStatMod.engine().removeByKey(player.getUuid(), stat, key);
    ...
    private static int statClear(ServerCommandSource source, ServerPlayerEntity player, String key) {
        int removed = 0;
        for (NativeStatEngine.StatInstance instance : SVFrameLibStatMod.engine().instances(player.getUuid())) removed += instance.removeIf(modifier -> modifier.key().equals(key));
    ...
    private static NativePlayerData requireData(ServerCommandSource source, ServerPlayerEntity player) {
        NativePlayerData data = SVFrameLibPlayerDataMod.getOrNull(player.getUuid());
        if (data == null) source.sendError(Text.literal("SVFrameLib player data is not loaded for " + player.getGameProfile().getName()));
        return data;
    }

    private static int healthScaleSet(ServerCommandSource source, ServerPlayerEntity player, double scale) { SVFrameLibHealthScale.setScale(player, scale); success(source, "Health scale for " + player.getGameProfile().getName() + " set to " + scale); return 1; }
    private static int healthScaleReset(ServerCommandSource source, ServerPlayerEntity player) { SVFrameLibHealthScale.resetScale(player); success(source, "Health scale for " + player.getGameProfile().getName() + " reset to config/default behavior."); return 1; }

    private static int dumpStats(ServerCommandSource source, ServerPlayerEntity player) {
        if (SVFrameLibStatMod.engine().instances(player.getUuid()).isEmpty()) { success(source, player.getGameProfile().getName() + " has no materialized stat instances."); return 1; }
        for (NativeStatEngine.StatInstance instance : SVFrameLibStatMod.engine().instances(player.getUuid())) success(source, instance.stat() + "=" + instance.formatFinal() + " modifiers=" + instance.modifiers().size());
        return 1;
    }
    ...
    private static boolean permitted(ServerCommandSource source, String permission) { Entity entity = source.getEntity(); return entity instanceof ServerPlayerEntity player ? SVFrameLibPermissionBridge.has(player, permission) : source.hasPermissionLevel(2); }
