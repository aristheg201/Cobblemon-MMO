...
import vn.svframe.svframelib.fabric.runtime.SVFrameLibWorkbenchLayout;
...
/** Native server-side SVFrameLib 1.7.1 Super/Mega Workbench implementation. */
public final class SVFrameLibWorkbenchMod {
    private static final Map<UUID, Session> SESSIONS = new ConcurrentHashMap<>();
    private static volatile boolean initialized;
    private SVFrameLibWorkbenchMod() {}

    public static void initialize() {
        if (initialized) return;
        initialized = true;
        ServerTickEvents.END_SERVER_TICK.register(SVFrameLibWorkbenchMod::tick);
    }
    public static boolean openSuper(ServerPlayerEntity player) { return open(player, SVFrameLibWorkbenchLayout.superWorkbench(), "Super Workbench"); }
    public static boolean openMega(ServerPlayerEntity player) { return open(player, SVFrameLibWorkbenchLayout.megaWorkbench(), "Mega Workbench"); }
    ...
    private static boolean open(ServerPlayerEntity player, SVFrameLibWorkbenchLayout layout, String title) {
    ...
    private static void craftResult(ServerPlayerEntity player, Session session, boolean craftToCompletion) {
        int crafted = 0;
        do {
            SVFrameLibCraftingRuntime.SlotAccess access = slotAccess(session);
            var match = SVFrameLibCraftingRuntime.match(SVFrameLibCraftingRuntime.Station.CUSTOM, stationKey(session.layout), access, permission -> SVFrameLibPermissionBridge.has(player, permission));
            ...
            if (craftToCompletion) {
                ...
                if (!remaining.isEmpty()) throw new IllegalStateException("SVFrameLib workbench capacity preflight diverged for " + match.get().recipe().id());
            } else {
    ...
    private static SVFrameLibCraftingRuntime.SlotAccess slotAccess(Session session) {
    ...
    private static void refreshResult(Session session) {
        session.inventory.setStack(session.layout.resultSlot(), SVFrameLibCraftingRuntime.result(SVFrameLibCraftingRuntime.Station.CUSTOM, stationKey(session.layout), slotAccess(session)));
    }
    private static int logicalToRaw(SVFrameLibWorkbenchLayout layout, int logicalSlot) {
        int[] input = layout.inputSlots(); return logicalSlot < 0 || logicalSlot >= input.length ? -1 : input[logicalSlot];
    }
    private static String stationKey(SVFrameLibWorkbenchLayout layout) { return layout.kind() == SVFrameLibWorkbenchLayout.Kind.SUPER ? "swb" : "mwb"; }
    ...
    private static final class Session {
        final SVFrameLibWorkbenchLayout layout;
        final SimpleInventory inventory;
        volatile ScreenHandler handler;
        private Session(SVFrameLibWorkbenchLayout layout, SimpleInventory inventory) { this.layout = layout; this.inventory = inventory; }
    }
}
