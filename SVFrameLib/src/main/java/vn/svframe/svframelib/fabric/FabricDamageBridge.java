...
public final class FabricDamageBridge {
    private static final Logger LOG = Logger.getLogger("SVFrameLib-Fabric/Combat");
    private static final Path CONFIG = FabricLoader.getInstance().getConfigDir().resolve("SVFrameLib").resolve("config.yml");
    private static volatile SVFrameLibDamageSettings settings = SVFrameLibDamageSettings.defaults();
    ...
    public static boolean reload() {
        ...
        try {
            settings = SVFrameLibDamageSettings.load(CONFIG);
        } catch (Exception e) {
            settingsLoaded = false;
            LOG.log(Level.SEVERE, "Failed to load SVFrameLib damage settings", e);
        }
        return SVFrameLibCombatRuntime.reload() & settingsLoaded;
    }
    ...
    public static String summary() {
        ...
        return "providers=" + StatProviderRegistry.providerCount()
                ...
                + "," + SVFrameLibCombatRuntime.summary();
    }
     ...
     * reduction are deliberately executed by SVFrameLibCombatRuntime in the same
    ...
    /** Native 1.21.1 equivalent of server-plugin platform EntityDamageEvent.DamageCause classification. */
    ...
    private static List<DamageType> classify(DamageSource source, SVFrameLibDamageSettings value) {
