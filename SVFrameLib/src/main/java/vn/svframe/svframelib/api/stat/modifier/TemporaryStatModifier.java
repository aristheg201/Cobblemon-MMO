...
public class TemporaryStatModifier extends StatModifier implements Closeable {
    ...
    public synchronized void register(MMOPlayerData data,long delayTicks){if(active)throw new IllegalStateException("Modifier is already active");if(delayTicks<0)throw new IllegalArgumentException("delayTicks must be >= 0");this.data=data;this.duration=delayTicks;this.startTime=System.currentTimeMillis();long tick=SVFrameLibFabricMod.currentTick();long expires=delayTicks>Long.MAX_VALUE-tick?Long.MAX_VALUE:tick+delayTicks;NativeStatEngine.Modifier n=toNative();SVFrameLibStatMod.engine().register(data.getUniqueId(),getStat(),new NativeStatEngine.Modifier(n.id(),n.key(),n.value(),n.type(),n.slot(),n.source(),expires));active=true;}
