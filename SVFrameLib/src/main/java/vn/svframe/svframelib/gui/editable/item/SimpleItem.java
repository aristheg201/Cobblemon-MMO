...
public class SimpleItem<T extends GeneratedInventory> extends PhysicalItem<T> {
    ...
    public SimpleItem(Map<String,?> config){this(null,config);} public SimpleItem(InventoryItem<T> parent,Map<String,?> config){super(parent,config);Object s=config==null?null:config.get("script");script=s==null?null:vn.svframe.svframelib.MythicLib.plugin.getSkills().loadScript("gui-"+System.identityHashCode(this),s);}
