...
public final class PlaceholderIntegration {
    ...
    public static void registerIfPresent() {
        ...
        add("fused", (id, s) -> Boolean.toString(s != null));
        add("fusion_type", (id, s) -> s == null ? "none" : s.typeName());
        add("tier", (id, s) -> s == null ? "none" : s.tier().name().toLowerCase(Locale.ROOT));
        add("pokemon", (id, s) -> s == null ? "" : s.pokemonName());
        add("species", (id, s) -> s == null ? "" : s.speciesId());
        add("bonus_percent", (id, s) -> s == null ? "0" : Integer.toString((int) Math.round(s.tier().multiplier() * 100d)));
        add("fusion_seconds", (id, s) -> s == null || !s.expires() ? "-1" : Long.toString((s.remainingTicks(SVFrameMMO.currentTick()) + 19L) / 20L));
        ...
        for (int slot = 0; slot < 4; slot++) {
            ...
            add("move_" + (slot + 1), (id, s) -> s == null || index >= s.moveIds().size() ? "" : s.moveIds().get(index));
        }
