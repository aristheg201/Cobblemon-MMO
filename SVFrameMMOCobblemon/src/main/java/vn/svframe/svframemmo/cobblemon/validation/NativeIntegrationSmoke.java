    ...
    public static void main(String[] args) {
        ...
        require(swordsDance.stages().stream().anyMatch(c ->
                        c.target() == MoveSemantic.Target.SELF
                                && c.stat() == BattleStat.ATTACK
                                && c.stages() == 2),
                "Swords Dance must be Attack +2");
        MoveSemantic lick = registry.resolveKnown("lick");
        require(lick.status() == MoveSemantic.Status.PARALYSIS && close(lick.statusChance(), 0.30),
                "Lick must preserve 30% paralysis");
        MoveSemantic bite = registry.resolveKnown("bite");
        require(bite.status() == MoveSemantic.Status.FLINCH && close(bite.statusChance(), 0.30),
                "Bite must preserve 30% flinch");
        ...
        require(CobblemonMoveSkillAdapter.isCanonicalSkillId(canonical),
                "temporary move identity recognition");
        require(CobblemonMoveSkillAdapter.moveIdFromCanonical(canonical).equals("thunderbolt"),
                "canonical id must round-trip to move id");
        require(close(BattleStat.multiplier(2), 2.0), "+2 stage multiplier must be 2x");
        require(close(BattleStat.multiplier(-1), 2.0 / 3.0),
                "-1 stage multiplier must be 2/3x");

        require(CosmeticDefinition.Slot.parse("aura") == CosmeticDefinition.Slot.AURA,
                "player cosmetics must use data-driven slots");
        require(CosmeticDefinition.Slot.parse("trail") == CosmeticDefinition.Slot.TRAIL,
                "trail must be an explicit cosmetic slot");
        require(CosmeticDefinition.Slot.AURA.defaultAnchor() == CosmeticDefinition.Anchor.BODY,
                "aura must default to the live body anchor");
        require(CosmeticDefinition.Slot.TRAIL.defaultAnchor() == CosmeticDefinition.Anchor.FEET,
                "trail must default to the movement/feet anchor");

        FusionCooldowns cooldowns = new FusionCooldowns();
        UUID id = UUID.randomUUID();
        cooldowns.markPotara(id, 10);
        require(cooldowns.potaraRemainingMillis(id) > 0L,
                "Potara cooldown must be timestamp based");
        cooldowns.markDance(id, 15 * 60);
        require(cooldowns.danceRemainingMillis(id) > 14L * 60L * 1000L,
                "Fusion Dance cooldown must support 15 minutes");
